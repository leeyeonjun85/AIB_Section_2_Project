...
## 프로젝트 설명서
### 프로젝트 개요 📝
이연준님은 Section 2에서 다양한 데이터셋을 활용한 머신러닝 방법론을 공부했습니다. 익숙한 데이터부터 전혀 도메인 지식이 없는 데이터까지 모두 다뤄봤습니다.
이번 프로젝트에선 이연준님이 **직접 선택한 데이터셋을 사용해서 만든 머신러닝 예측 모델을 통한 성능 및 인사이트를 도출/공유하는 것이 목표**입니다.

### 프로젝트 목표🔥
Section 2 프로젝트의 목표는 개요에서 알려드린대로 데이터셋을 사용하여 머신러닝 모델을 만든 후 성능 및 인사이트를 도출 / 공유하는 것으로 데이터셋 선정부터 모델 해석까지의 결과를 보여주는 것입니다.

데이터 전처리/EDA 부터 모델을 해석하는 과정을 코드로 작성하고, 해당 내용을 설명하는 영상을 제작하는 것이 이번 프로젝트 기간 동안 이연준님이 수행하셔야 하는 태스크입니다. 당연히 어떠한 코드, 분석, 라이브러리 혹은 목표를 가지는지는 온전히 이연준님의 자유입니다. 그러나, Section 2 Project인 만큼 해당 기간 동안 배운 내용을 위주로 설정하는 것이 권장됩니다.
...
추가로, 이연준님의 발표를 듣는 사람은 비데이터 직군이라 가정합니다.
즉 이연준님의 생각이나 가정들을 설명하는 과정에서 최대한 배경지식이 없는 사람들도 이해할 수 있도록 노력하시기 바랍니다!
...
❗ 데이터의 전처리 및 cleaning 과정은 모델링 성능에 큰 영향을 줍니다. 이연준님이 선정한 Task에 맞게 다양한 처리 방법을 조사 후 적용해 보세요.
...
## 프로젝트 체크리스트
아래는 프로젝트 체크리스트입니다. 기본 항목을 모두 만족하셔야 2점을 받으실 수 있습니다.

추가로 기본 항목을 모두 만족하고 [+] 항목까지 만족했다면 더 높은 점수를 받으실 수 있습니다. 프로젝트를 진행하시면서 기본 항목을 모두 만족하신 분들은 Advanced Level에 도전해보세요!

e.g.1 [+] 부분을 모두 만족하였지만 기본 항목 중 baseline 모델을 만들지 않았다. → 1점(최대 3점 중) → 기본 항목인 baseline 모델을 만들지 않았기 때문.

e.g.2 [+] 부분을 모두 만족하였지만 기본 항목 중 교차검증이나 hold-out 검증 방법을 사용해 데이터를 분리하지 않았다. → 1점(최대 3점 중) → 기본 항목인 검증 데이터 분리와 관련된 설명이 없기 때문.

e.g.3 기본 항목을 모두 만족하고 [+] 부분까지 만족했다. → 3점
...
### 제출 시 유의사항 체크
    - [ ] 5~10분 이내로 동영상을 촬영하여 제출하였다.
    - [ ] 제출 파일 이름을 AI_00_김코딩_section2.mp4로 설정하였다.
    - [ ] UrClass 제출폼(드롭박스)에 동영상 파일만 제출하였다. (코드 파일 아님)
    - [ ] 프로젝트에 사용한 파일을 나의 GitHub Repository(public)에 업로드하고 제출폼에 링크를 제출하였다.
...
### 프로젝트 발표 준비
프로젝트 발표영상은
여러분이 지원한 회사에 사전과제(입사과제)를 받아서 수행한 뒤,
면접관에게 해당 내용을 설명한다라는 상황을 가정하여 제작하시면 됩니다.
발표를 위한 준비물은 크게 세가지 입니다.

>발표 슬라이드 (Optional이며, Colab 노트북을 보면서 해도 괜찮습니다.)  
>발표 스크립트 (Optional)  
>발표 영상  
...
2) 발표 스크립트  
발표 슬라이드를 설명하는 영상을 만들기 위한 스크립트를 작성합니다.
한번에 영상을 촬영할 수 있다 라고 하면 만들지 않아도 좋습니다만, 스크립트가 있는 것과 없는 것의 차이는 생각보다 큽니다.
실제로 말을 하듯 스크립트를 작성하면 조금 더 자연스러운 결과물을 완성 할 수 있습니다.

3) 발표 영상 (녹화 및 제출)  
발표 영상은 발표 슬라이드와 스크립트를 활용해서 Zoom을 통해 5-10분 이내 의 영상을 녹화합니다.
좋은 분석을 했더라도 해당 내용을 다른 사람들이 이해하지 못하면 의미가 없기 때문에 리허설을 비롯하여, 충분한 시간동안 연습해야할 겁니다.
이후에 녹화한 영상파일을 설문조사 폼을 통해 제출하게 됩니다.
프로젝트 마지막 날에 여러분의 결과들을 서로 평가하는 시간을 가지게 됩니다.
영상 촬영 방법은 AI Docs를 참조하세요.

4) 발표 영상 제작시 유의사항  
제출 마감시간 전까지 반드시 제출하시기 바랍니다. 제출 마감시간을 초과하여 제출하신 분들은 동기들의 피드백을 받을 기회를 상실하게 되오니 유의하여 주시기 바랍니다.
5~10분은 길수도 있지만 반대로 여러분이 프로젝트에 쏟은 노력의 시간에 비하면 찰나의 순간이기 때문에 그 시간안에 여러분은 프로젝트의 요점만을 담은 발표 영상을 만들어주셔야 합니다.
프로젝트 평가를 하는 평가관/면접관은 여러분의 발표를 오직 영상을 통해서 여러분을 보게 됩니다. 화려한 동영상 편집기술을 써야하는 것이 아니지만, 여러분이 직접 면접관 / 평가관 앞에서 발표를 한다는 마음가짐으로 진행해주시길 바랍니다.
...
5) 발표 영상 외부 유출 금지 안내  
어떤 형태로든 업로드 된 타인의 산출물에 대한 외부 유출을 엄격하게 금지합니다.
학습 산출물에 대한 소유권이 ‘참여자‘(수강생)와 ‘교육기관’ (코드스테이츠)에게 있기 때문에 저작권과 관련한 이슈가 발생 시 코드스테이츠에서 법적인 조치를 취할 수 있습니다.


## Section2 프로젝트 피드백 가이드
Section 2 동기 피드백 진행 방법
Notion 피드백 페이지

Project Review 일정 소개
프로젝트 피드백 (9시 ~ 17시)

조원의 프로젝트 영상에 대한 피드백을 노션 페이지에 작성하는 시간

Project Review (17시 ~ 18시)

조별로 모여 노션에 작성한 피드백을 공유한 뒤, 다 같이 모여 전체 회고 진행

조별 피드백 (40분) + 전체 회고 (20분)
조별 피드백 진행 방법
소회의실에 조별로 모여 피드백을 전달

노션 페이지에 적힌 명단 순서대로 피드백을 받습니다.
1인에게 할당된 피드백 시간은 10분입니다.
피드백을 받는 분이 본인 피드백 시간을 진행해주세요.
피드백은 가장 먼저 피드백을 작성한 사람부터 마지막 사람 순으로 진행해주세요.
🚨 40분이라는 한정된 시간동안 진행하는 만큼 시간을 최대한 지켜주세요.
- 주어진 시간 안에 피드백을 잘 전달하기 위해서는 전달할 내용을 미리 정리해두어야겠죠?
피드백 전달이 일찍 끝났을 때에는 다음 질문들에 대해 함께 이야기해보세요.
프로젝트를 진행하면서 어려웠던 점
프로젝트 개선 방향
노션 피드백 작성 방법
배정된 조의 페이지 내 피드백을 작성할 조원의 페이지로 들어갑니다.
각 조별 페이지는 해당 조에 배정된 인원만 접근이 가능합니다.

해당 조원의 페이지 안 템플릿을 클릭하여 본인의 이름을 적은 후, 토글 내에 피드백을 작성합니다.
피드백 시 가져야 할 태도
영상이 있는 조원에 대한 피드백은 예외없이 꼭 진행합니다.
피드백 역시 학습 과정이므로, 같은 기수의 동료로서, 배정된 조원의 프로젝트에 대해서는 꼭 피드백을 작성 후 이야기해주세요.

그 외 인원에 대한 피드백은 여러분의 자유입니다.

책임감을 가지고 피드백을 작성합니다.
‘개선할 부분이 없어요’, ‘별 의견 없습니다’ 라고 말하는 것은 오히려 함께 고생한 동기에게 관심이 없다는 의미일 수 있습니다. 해당 동기의 프로젝트 영상을 꼼꼼히 확인하면서 어떤 점이 좋았는지 또는 어떤 점을 개선할 수 있을지 판단해주세요. 아주 조그마한 것이라도 개선할 수 있다면 동기에게 큰 도움이 될 것입니다. 혹은 해당 동기의 영상을 보면서 궁금했던 부분을 적어두고 피드백 시간에 이에 대해 이야기해보셔도 좋습니다.

상처가 되는 말은 사용하지 않도록 주의합니다.
프로젝트 피드백의 목적은 여러분의 성장이지 경쟁이 아닙니다. 다음 번 프로젝트를 더 잘할 수 있도록 솔직하고 명확하게 이야기하되, 감정이 상하지 않는 표현을 사용해주세요. 예를 들어, 누구보다 더 낫거나 부족하다는 식의 표현은 지양해주세요.

피드백을 무조건적으로 받아들일 필요는 없습니다.
본인의 프로젝트에 대해 가장 잘 아는 사람은 바로 자기 자신입니다. 따라서 본인에게 주어진 모든 피드백을 반드시 수용해야 하는 것은 아니며, 주는 사람도 본인의 의견이 받아들여지지 않는다고 해서 불쾌해하거나 걱정할 필요가 없습니다.

무조건적인 수용보다 피드백을 통해 다양한 관점에서 본인의 프로젝트를 바라보고, 더 나은 방향으로 발전시켜 나갈 수 있는 실마리를 가져가시기 바랍니다.
